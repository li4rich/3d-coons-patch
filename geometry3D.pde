...
Boolean twistFree=false, animating=true, center=true, showControlPolygon=true, showFloor=false;
float t=0, s=0;
  ...
  pushMatrix();   // to ensure that we can restore the standard view before writing on the canvas
    ...
    if(showControlPolygon) {
      ...
      fill(red,100); P.showPicked(6); // shows currently picked vertex in red (last key action 'x', 'z'
        ...
        drawBorders(P,Q,.003,animating,s);
        pen(green,1);fill(green);
        P.drawBalls(4);
        pen(red,1);fill(red);
        Q.drawBalls(4);
      }
    ...
    pen(cyan,0);fill(cyan); if(animating) {PtQ.setToL(P,s,Q); shadeSurface(PtQ.G,.005); fill(metal); show(coons(PtQ.G,u,v),5);}
...
void keyPressed() {
  ...
  if (key=='c') showControlPolygon = !showControlPolygon;
...
String title ="2014: Coons patch editor & animator in 3D", name ="Rich Li",
